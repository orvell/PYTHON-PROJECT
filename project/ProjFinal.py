...
def open_entry():
    ...
    l4 = Label(top, text="Hospital  :", bg="light cyan",font=("Helvetica", 9,"bold"))
    ...
    l4.place(x=170, y=400, height=25, width=60)
    ...
    e4 = Entry(top, textvariable=hospital).place(x=250, y=403)
    ...
    def submit():
        ...
        if (a!="" and b!="" and c!="" and d!="" and e!="" and f!=""):
            sql = "INSERT INTO patients (Testno ,Address ,Nameofpatient,Datefound,Age ,Hospital) VALUES (%s, %s,%s, %s,%s, %s)"
            val = (a, b, c, d, e, f)
...
def open_edit():
    ...
    def editing():
        ...
        if N in nos:
            ...
            l3 = Label(top1, text="Hospital  :",  bg="light cyan",font=("Helvetica", 9,"bold"))
            ...
            e3 = Entry(top1,textvariable=hospital).place(x=250, y=303) 
            ...
            def submit():
                ...
                if (b!="" and c!="" and d!="" and e!="" and f!=""):
                    ...
                    mycursor.execute(sql, val)
                    mydb.commit()
                    sql = "UPDATE patients SET Hospital=%s WHERE Testno=%s"
                    val = (f,N)
                    mycursor.execute(sql, val)
                    mydb.commit()
                    mydb.close()
...
def open_view():
    ...
    top.configure(bg="light cyan")
    l1 = Label(upper, text="Test No                              Name of patient                             Address                            Age                          Hospital                 Date ", bg="blue2",fg="lightgrey",font=("Helvetica", 8,"bold"))
    l1.pack(fill=X)
    mydb = mysql.connector.connect(host="localhost", user="root", passwd="orvell1830", database="db1") # change password here
    mycursor = mydb.cursor()
    ...
    mycursor.execute("SELECT Hospital FROM patients")
    records=mycursor.fetchall()
    print_records= ''
    for record in records:
        print_records += str(record[0])+"\n"
...
def open_help():
    ...
    l10 = Label(top, text=" ,Age int NOT NULL,Hospital text NOT NULL)", bg="light cyan",anchor=W)
...
cas.place(x=270, y=53, height=25, width=200)
cas.config(font=("Helvetica", 10,"bold"))
...
def cases():
    ...
    nos=list(sum(check,()))
    x=len(nos)
    lab=Label(root,text=x,bg="light cyan")
    lab.place(x=470,y=53, height=25)
